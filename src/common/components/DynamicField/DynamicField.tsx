...
export const DynamicField = memo(({ currentTitle, setValue, className }: Props) => {
    ...
    return editing ? (
        <input onChange={onChangeHandler} value={newTitle} onBlur={setValueHandler} autoFocus />
    ) : (
        <span
            style={{ lineBreak: "normal" }}
            className={`${className ? className + " " + s.dynamicSpan : s.dynamicSpan}`}
            onDoubleClick={changeHandler}
        >
            {currentTitle}
        </span>
    )
